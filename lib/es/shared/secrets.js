...
export function getAwsCredentials(profile) {
...
export async function setParameter(Name, Value, options = { Type: "SecureString", Overwrite: false }) {
    const ssm = await getSSM();
    return new Promise((resolve, reject) => {
...
export async function listParameters(options = {}) {
    const ssm = await getSSM();
    return new Promise((resolve, reject) => {
        const options = {};
...
export async function removeParameter(Name, options = {}) {
    const ssm = await getSSM();
...
export async function getServerlessConfig() {
    ...
    const config = yaml.safeLoad(readFileSync(`${process.env.PWD}/serverless.yml`, { encoding: "utf-8" }));
    return config;
}
