...
(async () => {
  ...
  if (currentVersion === npmVersion) {
    console.log(
      chalk.red.bold(
        `- Versions are the same, update your package.json before deploying đŠ`
      )
    );
  } else {
    try {
      await asyncExec(`yarn publish --new-version ${currentVersion}`);
      console.log(chalk.green.bold(`- published to npm successfully đ\n`));
    } catch (e) {
      console.log(
        chalk.red.bold(`\n- problems publishing to npm: ${e.code}  đĄ `)
      );
    }
  }
})();
